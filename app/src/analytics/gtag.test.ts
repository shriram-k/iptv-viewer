...
import { loadGtag, track, isGtagLoaded } from './gtag'
...
describe('loadGtag', () => {
  ...
  it('with an id: injects the tag, sets analytics_storage granted, disables auto page_view', () => {
    ...
    // dataLayer carries the consent default (analytics granted, ad_* denied) + config
...
describe('track', () => {
  ...
  it('emits an event once GA is loaded', () => {
    vi.stubEnv('VITE_GA4_MEASUREMENT_ID', 'G-TEST123')
    loadGtag()
